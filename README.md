...
| **Decision tree (idea)** | A chain of **if–else** rules (e.g. _if_ late count high _then_ higher risk branch).                                                                |
...
| **Outlier detection (IQR)**     | Rule: flag below **Q1 − 1.5×IQR** or above **Q3 + 1.5×IQR**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `outlier_detection_iqr.csv`.                                                                                                                                    | Debt ratio has about **~21%** rows flagged; age is **<0.04%**. Delinquency counts with many zeros produce tight fences, so many flagged points are not necessarily data errors.DebtRatio: many rows are far from the typical middle range (very heavy tail).                                                                             |

age: almost no extreme values (age is relatively stable).
For delinquency counts, many zeros make Q1 and Q3 small (sometimes both 0), so the rule can flag many non-zero values as outliers. |
...
- **What the data shows:** A **ranking** of which raw inputs the forest relied on most for _this_ model and random seed. It is **not** proof of business causality, but it tells you what the algorithm treated as most informative in the baseline.
...
_Re-run `python eda_credit_scoring.py` before a grade or presentation so figures, `eda_summary.txt`, and CSVs match one run._
